...
function createPuzzle() {
    ...
    setTimeout(enableTouchDnD, 100); // Mobil touch DnD
}
...
// Mobilde sürükle-bırak için touch desteği
function enableTouchDnD() {
    ...
    document.querySelectorAll('.puzzle-piece').forEach(piece => {
        ...
        piece.ontouchmove = function(e) {
            ...
            if (el && el.classList.contains('puzzle-piece') && el !== dragged) {
                el.parentElement.insertBefore(dragged, el);
            } else if (el && el.id === 'puzzleArea') {
                el.appendChild(dragged);
            } else if (el && el.id === 'piecesArea') {
                el.appendChild(dragged);
            }
            e.preventDefault();
        };
